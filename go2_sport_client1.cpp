...
{
    time_seg = 0.3;           // 每段时间长度
    time_temp = ct - time_seg; // 初始时间

    // 创建CSV文件来保存路径数据
    std::ofstream file("trajectory_circlev2.csv");
    if (!file.is_open())
    {
    ...
    // 写入CSV文件头
    file << "timeFromStart,x,y,yaw,vx,vy,vyaw" << std::endl;
    ...
    for (int i = 0; i < 30; i++)
    {
        ...
        // add
        radius = 0.6;  // 圆的半径
        angular_velocity = 0.2;  // 角速度

        // 生成路径点的局部坐标
        // px_local = 0;
        // py_local = 0.5 * sin(0.5 * time_temp);
        // yaw_local = 0;
        // vx_local = 0;
        // vy_local = 0.5 * cos(0.5 * time_temp);
        // vyaw_local = 0.5;

        px_local = radius * cos(angular_velocity * time_temp);
        // px_local = 0;
        py_local = radius * sin(angular_velocity * time_temp);
        // py_local = 0;
        yaw_local = angular_velocity * time_temp + M_PI_2;  // 偏航角随时间变化，保证始终朝向切线方向

        vx_local = -radius * angular_velocity * sin(angular_velocity * time_temp);
        // vx_local = 0;
        vy_local = radius * angular_velocity * cos(angular_velocity * time_temp);
        // yaw_local = atan2((vx_local * cos(yaw0) - vy_local * sin(yaw0)), (vx_local * sin(yaw0) + vy_local * cos(yaw0)));
        // vy_local = 0;
        vyaw_local = angular_velocity;  // 偏航角速度保持为角速度
        ...
        // 将路径点写入CSV文件
        file << path_point_tmp.timeFromStart << ","
             << path_point_tmp.x << ","
             << path_point_tmp.y << ","
             << path_point_tmp.yaw << ","
             << path_point_tmp.vx << ","
             << path_point_tmp.vy << ","
             << path_point_tmp.vyaw << std::endl;
    }
